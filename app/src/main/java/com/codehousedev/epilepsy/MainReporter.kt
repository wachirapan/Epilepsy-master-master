package com.codehousedev.epilepsy

import android.content.Intent
import android.database.Cursor
import android.support.v7.app.AppCompatActivity
import android.os.Bundle
import android.support.design.widget.NavigationView
import android.support.v4.view.GravityCompat
import android.support.v7.app.ActionBarDrawerToggle
import android.view.MenuItem
import android.widget.Button
import android.widget.ListView
import android.widget.TextView
import android.widget.Toast
import com.codehousedev.data.SymptomsData
import com.codehousedev.sqlite.QuerySQLite
import com.codehousedev.sqlite.UpdateSQLite
import com.codehousedev.submain.SubClassReporter
import kotlinx.android.synthetic.main.activity_main.*
import kotlinx.android.synthetic.main.app_bar_main.*

class MainReporter : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    lateinit var mnameprofile : TextView
    lateinit var mlastnameprofile : TextView
    lateinit var mageprofile : TextView
    lateinit var mainsymtom : TextView
    lateinit var mlistreporter : ListView
    lateinit var mlist : ArrayList<SymptomsData>
    lateinit var mcursor : Cursor
    lateinit var mcheckcomplete : Button
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_main_reporter)
        setSupportActionBar(toolbar)
        val toggle = ActionBarDrawerToggle(
                this, drawer_layout, toolbar, R.string.navigation_drawer_open, R.string.navigation_drawer_close)
        drawer_layout.addDrawerListener(toggle)
        toggle.syncState()

        nav_view.setNavigationItemSelectedListener(this)

        initdata()
        queryprofile()
        querylist()
        updatesymptom()
    }
    fun initdata()
    {
        mnameprofile = findViewById(R.id.nameprofile)
        mageprofile = findViewById(R.id.ageprofile)
        mainsymtom = findViewById(R.id.mainsymtom)
        mlistreporter = findViewById(R.id.listreporter)
        mcheckcomplete = findViewById(R.id.checkcomplete)

    }
    fun queryprofile()
    {
        var queryProfile = QuerySQLite(this)
        var mprofile : Cursor  = queryProfile.queryprofile()
        while (mprofile.moveToNext()){
            mnameprofile.setText(mprofile.getString(1))

            mageprofile.setText(mprofile.getString(2))
            mainsymtom.setText(mprofile.getString(6))
        }
    }
    fun querylist()
    {
        mlist = ArrayList()
        var querysymptom = QuerySQLite(this)
        mcursor = querysymptom.querysymptoms()
        while (mcursor.moveToNext()){
            var symptomid = mcursor.getInt(0)
            var headdetail = mcursor.getString(1)
            var datedetail = mcursor.getString(9)
            mlist.add(SymptomsData(symptomid!!, headdetail,datedetail))
        }
        var Subsymptom = SubClassReporter(this,mlist)
        mlistreporter.setAdapter(Subsymptom)

    }
    fun updatesymptom()
    {
        var update = UpdateSQLite(this)
        mcheckcomplete.setOnClickListener({
            update.updatecheckprofile()
            var completereport = Intent(this,MainAfterReport::class.java)
            startActivity(completereport)
        })
    }

    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        // Handle navigation view item clicks here.
        when (item.itemId) {
            R.id.nav_main -> {
                var intentmain = Intent(this,MainActivity::class.java)
                startActivity(intentmain)
            }

            R.id.nav_slideshow -> {
                var intentsymptoms = Intent(this,MainSymptoms::class.java)
                startActivity(intentsymptoms)
            }
            R.id.nav_manage -> {
                var intentsettime = Intent(this,MainSettime::class.java)
                startActivity(intentsettime)
            }
            R.id.nav_share -> {
                var intentreporter = Intent(this,MainReporter::class.java)
                startActivity(intentreporter)
            }
            R.id.nav_send ->{
                var intentafterReport = Intent(this,MainAfterReport::class.java)
                startActivity(intentafterReport)
            }

        }

        drawer_layout.closeDrawer(GravityCompat.START)
        return true
    }
}
